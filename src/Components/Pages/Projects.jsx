...
function Projects() {
  ...
  const handleModify = id => userIsSuper ? navigate(`/modify-project/${id}`) : alert("Only superusers can modify projects.");
  ...
  return (
    <div className="projects-page">
      <header className="projects-header">
        <h2>Projects</h2>
        {userIsSuper && <button onClick={() => navigate("/add-project")}>+ Add Project</button>}
      </header>
      {loading ? <p>Loading projects...</p> : projects.length === 0 ? <p>No projects</p> : (
        <div className="projects-grid">
          {projects.map(project => (
            <article key={project.id} className="project-card">
              {project.image ? <img src={safeImage(project.image)} alt={project.title} onError={e => e.target.src = "https://via.placeholder.com/420x240?text=No+Image"} /> : <div>No image</div>}
              <h3>{project.title}</h3>
